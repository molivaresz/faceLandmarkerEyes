...
def drawing_output(frame, coordinates_left_eye, coordinates_right_eye, blink_counter):
    ...
    cv2.rectangle(output, (0, 0), (200, 50), (175, 109, 74), -1)
    cv2.rectangle(output, (202, 0), (265, 50), (175, 109, 74), 1)
    cv2.putText(output, "Nro. Parpadeos:", (10, 30), cv2.FONT_HERSHEY_DUPLEX, 0.7, (150, 150, 150), 2)
    cv2.putText(output, "{}".format(blink_counter), (220, 35), cv2.FONT_HERSHEY_DUPLEX, 0.9, (0, 0, 197), 2)
        ...
        if results.multi_face_landmarks is not None:
            for face_landmarks in results.multi_face_landmarks:
                for index in index_left_eye:
                    ...
                    cv2.circle(frame, (x, y), 2, (0, 255, 255), 0)
                    cv2.circle(frame, (x, y), 1, (128, 0, 250), 0)
                for index in index_right_eye:
                    x = int(face_landmarks.landmark[index].x * width)
                    y = int(face_landmarks.landmark[index].y * height)
                    coordinates_right_eye.append([x, y])
                    cv2.circle(frame, (x, y), 2, (128, 0, 250), 0)
                    cv2.circle(frame, (x, y), 1, (0, 255, 255), 0)
